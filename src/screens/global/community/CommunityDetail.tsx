...
type NavProp = NavigationProp<StackParamList, "CommunityDetailScreen">;
...
const CommunityDetailScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const [communitySnapshot, communityLoading, communityError] =
    useDocument(communityRef);
  ...
  async function joinCommunity(which: string) {
    ...
    which === "join"
      ...
      : (members = community.members.filter((it: string) => {
          it !== user.uid;
        }));
    const communityRef = doc(firestore, "communities", community.id);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.surface }}>
      <Loader
        showLoader={
          values.loading || loading || communityLoading || postsLoading
        }
      />
      ...
      <View style={{ flex: 1 }}>
            ...
            <View style={{ height: height * 0.35 }}>
              ...
              >
                ...
                >
                  ...
                  >
                    <Text style={{ ...TYPOGRAPHY.p }}>{`${followerCount(
                    ...
                    {community.members.includes(user.uid) ? (
                      ...
                      >
                        <Text
                          style={{ ...TYPOGRAPHY.p, color: community.color }}
                        >
                          Join
                        </Text>
                      </TouchableOpacity>
        ...
        {posts.length === 0 && (
          ...
          <EmptyDesign 
            ...
            description={
              "There are no conversations in this community yet. Click the add button to get it started. You'll also need to join the community first if you aren't a member"
            }
