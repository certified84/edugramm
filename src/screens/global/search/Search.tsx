  ...
  useEffect(() => {
    if (error && error.message !== "") {
      setValues({
        ...values,
        showSnackBar: true,
        message: error.message ?? postsError.message,
      });
    }
  }, [error, postsError]);
      ...
      {values.posts.length > 0 && (
        <ImageDialog
          ...
          setShowImageDialog={(value) =>
            setValues({ ...values, showImageDialog: value })
          }
        ...
        {values.searching ? (
          <View style={{ flex: 1 }}>
            <View>
              ...
              >
                ...
                <View style={{ flex: 0.7 }}>
                  <TextInput
                    ...
                    onChangeText={(text) =>
                      setValues({ ...values, searchQuery: text })
                    }
                    mode="outlined"
                    placeholder="Search"
                    // left={
                ...
                <TouchableOpacity
                  onPress={() => setValues({ ...values, searching: false })}
                  activeOpacity={0.9}
                  style={{
                    flex: 0.2,
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <Account accountInfo={item.data()} navigation={navigation} />
                )}
          ...
          <View style={{ flex: 1 }}>
            {!values.posts || values.posts.length === 0 ? (
              ...
              <View style={{ flex: 1 }}>
                <TouchableOpacity
                  onPress={() => setValues({ ...values, searching: true })}
                  activeOpacity={0.9}
                  style={{
                    paddingVertical: SIZES.sm,
                    backgroundColor: COLORS.surface1,
                ...
                <FlatList
                  ...
                  renderItem={({ item, index }) => (
                    <Post
                      ...
                      index={index}
                      setCurrentIndex={(i) =>
                        setValues({ ...values, currentIndex: i })
                      }
                      setShowImageDialog={(value) =>
                        setValues({ ...values, showImageDialog: value })
                      }
