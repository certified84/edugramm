...
const PostDetailedCard = ({ item, navigation, userInfo }) => {
  ...
  async function likePost(isLiked: boolean) {
    ...
    await updateDoc(postRef, { likes: [...likes] }).catch((error) => {
      ...
      <View style={{ paddingHorizontal: SIZES.md, paddingVertical: SIZES.xs }}>
            ...
            onPress={() => {
              post.uid === user.uid
                ...
                : navigation.navigate("UserDetailScreen", {
                    userInfo: { ...userInfo, name: post.name, uid: post.uid },
                  });
            }}
          >
            <View
        ...
        <View style={{ flex: 1, marginTop: SIZES.sm }}>
          ...
          <View style={{ ...styles.bottomSection }}>
            <View
              style={{ flexDirection: "row", flex: 0.4, alignItems: "center" }}
            >
              <TouchableOpacity
                onPress={() => {
                  setLiked(!liked);
              ...
              >
                <AntDesign
                  name={post.likes.includes(user.uid) ? "heart" : "hearto"}
                  size={SIZES.md}
                  color={liked ? COLORS.red : COLORS.onSurface}
                />
              </TouchableOpacity>
